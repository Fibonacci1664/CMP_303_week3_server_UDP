...
int main()
{
	...
	// Create a UDP socket.
	SOCKET UDPserverSocket = socket(AF_INET, SOCK_DGRAM, 0);
	...
	if (bind(UDPserverSocket, (const sockaddr *) &serverAddr, sizeof(serverAddr)) != 0)
	{
		die("bind failed");
	}
	...
	while (true)
	{
		...
		int count = recvfrom(UDPserverSocket, buffer, MESSAGESIZE, 0,
		...
		if (sendto(UDPserverSocket, buffer, MESSAGESIZE, 0,
